...
const EmailVerify = ({ onStageChange }: stageProp) => {
    ...
    const verifyOTP = async () => {
        ...
        const isValid = otpCode.match(otp)
        if (isValid) {
        ...
        <div className="flex flex-col py-16 justify-center px-8 items-center">
            <div className="flex flex-col gap-8 w-full md:w-[460px] ">
                <div className="">
                    ...
                    <p>Please enter the 6-digit verification code that was sent. The code is valid for 30 minutes.</p>
                </div>
                <div className="flex flex-col gap-8">
                    <div className="flex flex-col gap-2">
                        <label htmlFor="uid" className="text-sm text-slate-500">Email Verification Code</label>
                        <div className="relative w-full flex items-center">
                            <input type="text" id="uid" value={otpCode} onChange={(e) => setOtpCode(e.target.value)} disabled className="border w-full text-gray-400 outline-0 px-3 py-3 text-xl font-semibold rounded-sm focus:border-primary-dark" />
                            <small className="absolute right-4 text-green-500 flex items-center gap-1"><InformationCircleIcon className="w-5 h-5" /> Auto-Verify enabled</small>
                        </div>
                    </div>
